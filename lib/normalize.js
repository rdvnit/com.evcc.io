...
function normalizeLoadpoint(lp, index) {
  ...
  const alwaysCharge = smartModeSchema
    ? ['off', 'on', 'once'].includes(lp.alwaysCharge) ? lp.alwaysCharge : 'off'
    : null;
  const alwaysChargeSupported = smartModeSchema
    && !Boolean(lp.chargerFeatureSwitchDevice)
    && !Boolean(lp.chargerFeatureContinuous);
  const rawMode = pick(lp, ['mode'], 'off');
  const homeyMode = rawMode === 'smart'
    ? alwaysCharge === 'on' || alwaysCharge === 'once' ? 'minpv' : 'pv'
    : rawMode;
